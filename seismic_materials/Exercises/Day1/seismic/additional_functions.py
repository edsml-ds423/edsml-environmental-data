...
def reflection_time(t0, x, vnmo):
    """
    Calculate the travel-time of a reflected wave at a certain depth, i using,
    
            ti = sqrt(t0^2 + (x^2 / v^2))
...
def sample_trace(trace, time, dt):
    ...
    if any(samples < 0) or any(samples >= N):
        ...
        interpolator = scipy.interpolate.CubicSpline(times, amps)
...
def nmo_correction(cmp, dt, offsets, velocities):
    ...
    for i, t0 in enumerate(times):
        for n_offset, x in enumerate(offsets):
            # loop through all of the data points (vertical) for each offset 
            # for each datapoint i, at ti, calculate the travel time using ti = root(t0^2 + x^2/v^2)
            # find the amplitude of that measurement idx (ti/dt) from the cmp gather for that offset
            # if the time t is outside of the CMP time range, amplitude will be None.
            t_i = reflection_time(t0=t0, x=x, vnmo=velocities[i]) # find the travel-time for a specific reflection on a trace
            try: 
                amplitude = cmp[:, n_offset][int(t_i/dt)] 
            except: 
                amplitude = None
            if amplitude is not None:
                nmo[i, n_offset] = amplitude
...
def semblance(vs, cmp, dt, offsets, ts): 
    """create semblance plot."""
    amplitudes = []
    for _, v in enumerate(vs): #iteratively try different velocities
        v_nmo = np.ones(len(ts))*v # array of constant velocity for each depth, i 
        nmo = nmo_correction(cmp, dt, offsets, v_nmo) # apply a nmo correction for the constant vmo array
        sum_row = np.sum(nmo, axis = 1) # sum the nmo along the x axis for each depth, i (1200 summed values)
        amplitudes.append(sum_row)
